  ...
  public class DfWrapper
  {
    ...
    public class DfApi
    {
      internal DfApi(object a)
      {
        try
        {
          ...
          LogFormatted("Success: " + (_apiReadyField != null).ToString());
          ...
          LogFormatted("Success: " + (_actualFrozenKerbals != null).ToString());
        }
        catch (Exception ex)
        {
      ...
      internal Dictionary<string, KerbalInfo> FrozenKerbals
      {
        get
        {
          ...
          object gamesettings = gamesettingsfield.GetValue(_actualDfapi);
          _actualFrozenKerbals = _frozenKerbalsMethod.Invoke(_actualDfapi, null);
      ...
      private Dictionary<string, KerbalInfo> ExtractFrozenKerbalDict(object actualFrozenKerbals)
      {
        ...
        try
        {
          foreach (object item in (IDictionary)actualFrozenKerbals)
          {
    ...
    public class DeepFreezer
    {
      internal DeepFreezer(object a)
      {
        ...
        _crewXferToActiveMethod = DeepFreezerType.GetMethod("get_DFIcrewXferTOActive", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance);
        CrewXferToActive = GetcrewXferToActive;
        _crewXferFromActiveMethod = DeepFreezerType.GetMethod("get_DFIcrewXferFROMActive", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance);
        ...
        LogFormatted_DebugOnly("Success: " + (_beginFreezeKerbalMethod != null).ToString());
        ...
        LogFormatted_DebugOnly("Success: " + (_beginThawKerbalMethod != null).ToString());
      }
      ...
      private MethodInfo _crewXferToActiveMethod;

      private bool GetcrewXferToActive
      {
        get { return (bool)_crewXferToActiveMethod.Invoke(_actualDeepFreezer, null); }
      }
      ...
      private MethodInfo _crewXferFromActiveMethod;

      private bool GetcrewXferFromActive
      {
        get { return (bool)_crewXferFromActiveMethod.Invoke(_actualDeepFreezer, null); }
      }
      ...
      /// <returns></returns>
      private static FrznCrewList ExtractStoredCrewList(object actualStoredCrewList)
      {
        ...
        try
        {
          ...
          foreach (object item in (IList)actualStoredCrewList)
          {
    ...
    internal static void LogFormatted(string message, params object[] strParams)
    {
      ...
      string strMessageLine =
        $"{DateTime.Now},{Assembly.GetExecutingAssembly().GetName().Name}-{MethodBase.GetCurrentMethod().DeclaringType.Name},{message}";
